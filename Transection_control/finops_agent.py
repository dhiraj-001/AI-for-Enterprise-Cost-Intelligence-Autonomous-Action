...
Core FinOps Agent that orchestrates:
  ...
  • Root-cause analysis (Rule-based heuristics)
...
class FinOpsAgent:
    ...
    # =========================================================================
    #  4.  ROOT CAUSE ANALYSIS (RULE-BASED)
    # =========================================================================
    def _generate_rule_based_insight(self, row: dict) -> str:
        """Deterministic, rule-based text generation for transaction insights."""
        status = row.get("status", "")
        tid = row.get("transaction_id", "Unknown")
        amt = row.get("amount", 0.0) if pd.notna(row.get("amount")) else 0.0
        bank_amt = row.get("bank_amount", 0.0) if pd.notna(row.get("bank_amount")) else 0.0
        var = row.get("amount_variance", 0.0) if pd.notna(row.get("amount_variance")) else 0.0
        sender = row.get("sender", "Unknown")
        receiver = row.get("receiver", "Unknown")
        
        if status == "amount_mismatch":
            return f"Ledger recorded ₹{amt:,.2f}, but Bank shows ₹{bank_amt:,.2f} (Variance: ₹{var:,.2f}). Check for hidden FX fees or vendor overcharges."
        elif status == "missing_in_bank":
            return f"₹{amt:,.2f} transfer from {sender} to {receiver} is missing from the bank statement. Likely a settlement delay or weekend lag."
        elif status == "missing_in_ledger":
            return f"Bank processed an unexpected ₹{bank_amt:,.2f} transaction. Possible unrecorded direct debit or unauthorized charge."
        elif status == "duplicate":
            return f"Bank statement shows multiple identical charges for {tid}. Investigate double-billing by vendor."
        elif status == "balance_inconsistency":
            return f"Math mismatch: {sender}'s starting/ending balances do not align with the ₹{amt:,.2f} transaction amount."
        elif status == "ml_anomaly":
            return f"Statistical outlier: ₹{amt:,.2f} deviates significantly from {sender}'s baseline behavior. Flagged for fraud review."
        elif status == "transaction_spike":
            return f"Volume spike: Unusually high frequency of transactions detected from {sender}."
        elif status == "matched_fuzzy":
            return f"Matched via fuzzy logic: Minor inconsistencies found in sender/receiver names."
        else:
            return f"Manual review required for status: {status}."

    def analyze_root_cause(self) -> pd.DataFrame:
        """Appends human-readable insights, recommended actions, and execution statuses."""
        logger.info("=== FinOpsAgent: analyze_root_cause() ===")
        if self.issues_df is None:
            self.detect_issues()

        # 1. Define the Root Cause Explanations
        cause_map = {
        ...
        # 2. Define the AUTONOMOUS ACTIONS (This makes it an Agent!)
        action_map = {
            "missing_in_bank":       "Hold Ledger Reconciliation",
            "missing_in_ledger":     "Query Bank API for Status",
            "amount_mismatch":       "Trigger Vendor Dispute Workflow",
            "duplicate":             "Auto-Block Duplicate Payment",
            "balance_inconsistency": "Flag Account for Audit",
            "ml_anomaly":            "Freeze Sender Account",
            "transaction_spike":     "Rate-Limit Sender",
            "matched_fuzzy":         "Auto-Merge Records",
        }

        self.issues_df["root_cause"] = self.issues_df["status"].map(cause_map).fillna("Unknown discrepancy")
        self.issues_df["recommended_action"] = self.issues_df["status"].map(action_map).fillna("Manual Review")
        
        # 3. Simulate the Execution Status (What the Agent actually did)
        def execute_action(action):
            if action in ["Auto-Block Duplicate Payment", "Auto-Merge Records", "Hold Ledger Reconciliation", "Rate-Limit Sender"]:
                return "EXECUTED"
            elif action in ["Freeze Sender Account", "Trigger Vendor Dispute Workflow"]:
                return "PENDING ADMIN APPROVAL" # High risk actions need a human-in-the-loop
            else:
                return "TICKET CREATED"

        self.issues_df["execution_status"] = self.issues_df["recommended_action"].apply(execute_action)

        # ── Rule-based granular analysis ─────────────────────────────────
        logger.info("Executing rule-based granular analysis for all items...")
        self.issues_df["genai_insight"] = "Analysis pending..." 
        
        for idx, row in self.issues_df.iterrows():
            insight = self._generate_rule_based_insight(row.to_dict())
            self.issues_df.at[idx, "genai_insight"] = insight

        logger.info("Root-cause analysis and action mapping complete.")
        return self.issues_df
    # =========================================================================
    #  5.  FINANCIAL IMPACT
    # =========================================================================
    def _generate_closing_narrative(self, summary: dict, top_3: pd.DataFrame) -> str:
        """Generates an executive summary paragraph dynamically."""
        total_var = summary.get("total_variance_inr", 0)
        total_issues = summary.get("total_issues", 0)
        
        narrative = (f"Executive Summary: The FinOps agent successfully reconciled {summary.get('total_transactions', 0):,} "
                     f"transactions, identifying {total_issues:,} discrepancies. This resulted in a total financial exposure "
                     f"of ₹{total_var:,.2f}. ")
        
        if not top_3.empty:
            narrative += "The highest impact issues driving this variance are: "
            top_issues_str = []
            for _, row in top_3.iterrows():
                amt = row.get('amount') if pd.notna(row.get('amount')) else row.get('bank_amount', 0)
                clean_status = row['status'].replace('_', ' ').title()
                top_issues_str.append(f"{row['transaction_id']} (₹{amt:,.2f} - {clean_status})")
            narrative += ", ".join(top_issues_str) + ". "
            
        narrative += "Immediate manual intervention is recommended for these high-value items to recover funds."
        return narrative
    ...
    def calculate_impact(self) -> dict:
        ...
        # ── Closing Narrative ────────────────────────────────────────────
        logger.info("Generating Closing Narrative...")
        top_3 = df.sort_values("amount", ascending=False).head(3)
        self.closing_narrative = self._generate_closing_narrative(self.impact_summary, top_3)
